...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .risk-alert.warn { border-left-color: var(--warning); background: var(--warning-bg); color: var(--warning-text); }
    ...
    <div class="container">
        <div class="page-header">
            ...
            <div style="display:flex; gap:8px; align-items:center;">
                <a class="theme-toggle" href="docs/parameter-erklaerung.html" target="_blank" rel="noopener" title="Alle Kennzahlen einfach erklärt" style="text-decoration:none;">ℹ️ Parameter erklärt</a>
                <a class="theme-toggle" href="docs/parameter-erklaerung.pdf" target="_blank" rel="noopener" title="Als PDF öffnen" style="text-decoration:none;">📄 PDF</a>
    ...
    (async () => {
        ...
        <table>
            <tr>
                ...
                <th title="Abstand zum engsten der drei Stops (Hard -20% / Relativ vs. DAX / Vola-Stop) — von der Engine berechnet">🛑 Stop-Abstand</th>
                <th>🤖 KI-Sentiment</th>
                <th>📊 Earnings</th>
            </tr>`;
        // Stop-Ampel (V2): Engine-berechneter Abstand zum engsten Stop.
        // 🔴 < 3pp, 🟡 3-8pp, 🟢 > 8pp. Kein Nachrechnen im Dashboard.
        function stopBadge(p) {
            const si = p.stop_info;
            if (!si || typeof si.abstand_pct !== 'number') return '<span style="color:var(--text-muted);">–</span>';
            const cls = si.abstand_pct < 3 ? 'sell' : (si.abstand_pct < 8 ? 'hold' : 'buy');
            const icon = si.abstand_pct < 3 ? '🔴' : (si.abstand_pct < 8 ? '🟡' : '🟢');
            const tip = `${si.typ} bei ${formatEUR(si.stop_kurs)} — Kurs müsste ${si.abstand_pct.toFixed(1).replace('.', ',')}% fallen`;
            return `<span class="badge ${cls}" title="${tip}" style="cursor:help;">${icon} ${si.abstand_pct.toFixed(1).replace('.', ',')}%</span>`;
        }
        ...
        function renderOverview() {
            ...
            // Stop-Ampel (V2): Positionen nahe am engsten Stop prominent warnen.
            const stopNah = (dep.positionen || [])
                .filter(p => p.stop_info && typeof p.stop_info.abstand_pct === 'number' && p.stop_info.abstand_pct < 8)
                .sort((a, b) => a.stop_info.abstand_pct - b.stop_info.abstand_pct);
            const stopAlerts = stopNah.map(p => {
                const si = p.stop_info;
                const pct = si.abstand_pct.toFixed(1).replace('.', ',');
